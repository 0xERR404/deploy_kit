...
# Единственный внешний вход — хаб NEXUS404 Hub на этом корневом
...
EOF
    ...
    # -----------------------------------------------------------------------
    # Базовый (корневой) домен. NEXUS404 Hub рассчитан на домен (без
    ...
    if [ -n "$EXISTING_DOMAIN" ]; then
        ...
        if [ "$DK_HAS_DOMAIN" = "no" ]; then
            ...
            echo "${YELLOW}[?]${NC} Без домена у NEXUS404 Hub не будет HTTPS-адреса,"
