...
# через NEXUS404 Interface (виджет в хабе, который сам подписывается на
...
if is_done "step5_3"; then
    ...
    if [ "$(dk_ntfy_mode)" != "host" ]; then
        ...
        # Токен для будущего виджета хаба (NEXUS404 Interface) — читает
...
if is_done "step5_4"; then
    ...
    # Безопасно вызывать, даже если хаб (NEXUS404 Interface) ещё не
...
if [ "$NTFY_MODE_NOW" = "host" ]; then
    ...
    echo "$(pad_field "Карточка в хабе:" "$FIELD_WIDTH")widget 'ntfy-feed' (см. NEXUS404 Interface)"
else
