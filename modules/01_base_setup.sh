#!/bin/bash
# =============================================================================
# Модуль 1: Базовая настройка сервера
# (DNS, swap, BBR, auditd, sudo-пользователь, SSH, ufw, fail2ban, apparmor,
#  NTP, лимиты, cron: reboot/cleanup/unattended-upgrades, health-checker,
#  финальная проверка и отчёт)
#
# Подключается из menu.sh через `source`, common.sh уже загружен.
# Reboot в самом конце оригинального скрипта убран по требованию —
# после завершения модуль просто возвращает управление в меню.
#
# STATEFILE: "step1_N".
# =============================================================================
...
# перезапуск sshd (специфично для этого модуля — общей SSH-логики другим
# модулям не требуется, поэтому функция живёт здесь, а не в common.sh)
restart_ssh() {
    # /run/sshd нужен для privilege separation — без него sshd -t падает
    # с ложной ошибкой "Missing privilege separation directory", даже если
    # сам конфиг корректен. Создаём ДО проверки синтаксиса.
    mkdir -p /run/sshd

    # проверка синтаксиса конфига ДО перезапуска — если конфиг битый,
    # перезапуск оборвёт доступ по SSH без возможности откатиться удалённо
    ...
    # Ubuntu 22.04+: ssh.socket слушает порт независимо от sshd_config — переводим на прямой демон
    if systemctl list-unit-files 2>/dev/null | grep -q '^ssh\.socket'; then
        systemctl disable --now ssh.socket >> "$LOGFILE" 2>&1 || true
        systemctl enable ssh.service >> "$LOGFILE" 2>&1 || systemctl enable sshd.service >> "$LOGFILE" 2>&1 || true
    fi

    run_spinner "Перезапуск SSH-службы" "systemctl restart sshd || systemctl restart ssh"
}

# Шаг 1 (подготовка рабочего окружения) убран — директория и служебные
# файлы уже создаются в common.sh при запуске меню.
...
if is_done "step1_1"; then
    ...
    DNS1=""
    while [ -z "$DNS1" ]; do
        read -rp "${YELLOW}[?]${NC} Выберите вариант (1-3): " DNS_CHOICE || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
...
if is_done "step1_5"; then
    ...
    NEW_USER=""
    while [ -z "$NEW_USER" ]; do
        read -rp "${YELLOW}[?]${NC} Введите имя нового sudo-пользователя: " INPUT_USER || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
    ...
    while true; do
        read -rsp "${YELLOW}[?]${NC} Введите пароль для '$NEW_USER' (минимум 8 символов): " NEW_PASS || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
        echo ""
        if [ "${#NEW_PASS}" -lt 8 ]; then
            echo "${RED}[!]${NC} Слишком короткий пароль (минимум 8 символов), попробуйте снова"
            continue
        fi
        read -rsp "${YELLOW}[?]${NC} Повторите пароль: " NEW_PASS_CONFIRM || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
        echo ""
    ...
    if [ -f /root/.ssh/authorized_keys ] && [ -s /root/.ssh/authorized_keys ]; then
        ...
        while [ -z "$ROOT_KEY_CHOICE" ]; do
            read -rp "${YELLOW}[?]${NC} Убрать SSH-ключи root? (y/n): " ROOT_KEY_CHOICE || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
...
if is_done "step1_6"; then
    ...
    while true; do
        read -rp "${YELLOW}[?]${NC} Введите новый SSH-порт (1024-65535): " NEW_PORT || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
    ...
    if [ "$CURRENT_PORT" != "$NEW_PORT" ]; then
        ufw delete allow "$CURRENT_PORT"/tcp >> "$LOGFILE" 2>&1 || true
    fi
    echo "${GREEN}[✓]${NC} Разрешён только новый SSH-порт $NEW_PORT (входящие остальные — запрещены)"
    ...
    confirm_or_exit "Подключение по новому порту работает?" "Прервано. Порт $NEW_PORT, бэкап: ${SSHD_CONFIG}.bak.*"

    echo "${GREEN}[✓]${NC} Новый SSH-порт и файрвол подтверждены"
...
if is_done "step1_7"; then
    ...
    while [ -z "$AUTH_CHOICE" ]; do
        read -rp "${YELLOW}[?]${NC} Выберите вариант (1 или 2): " AUTH_CHOICE || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
    ...
    if [ "$AUTH_CHOICE" == "2" ]; then
        ...
        while [ -z "$PASTED_KEY" ] && [ "$CANCELLED" -eq 0 ]; do
            read -rp "${YELLOW}[?]${NC} Публичный ключ: " KEY_INPUT || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
            if [ -z "$KEY_INPUT" ]; then
                CANCELLED=1
...
if is_done "step1_10"; then
    ...
    while [ -z "$SELECTED_TZ" ]; do
        read -rp "${YELLOW}[?]${NC} Выберите вариант (1-15): " TZ_CHOICE || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
        if [ "$TZ_CHOICE" == "15" ]; then
            read -rp "${YELLOW}[?]${NC} Введите часовой пояс (формат Region/City): " TZ_CUSTOM || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
...
EOF
    ...
    while [ -z "$REBOOT_DOW" ]; do
        read -rp "${YELLOW}[?]${NC} День недели для перезагрузки (пн/вт/ср/чт/пт/сб/вс): " DAY_INPUT || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
    ...
    while [ -z "$REBOOT_TIME" ]; do
        read -rp "${YELLOW}[?]${NC} Время перезагрузки в формате ЧЧ:ММ (например 04:00): " TIME_INPUT || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
...
EOF
    ...
    HC_TIME=""
    while [ -z "$HC_TIME" ]; do
        read -rp "${YELLOW}[?]${NC} Время ежедневной проверки в формате ЧЧ:ММ (например 06:00): " HC_TIME_INPUT || { echo "${RED}[!]${NC} Не удалось прочитать ввод"; exit 1; }
...
if is_done "step1_14"; then
    ...
    check_item "SSH включён в автозагрузку" bash -c "systemctl is-enabled --quiet sshd 2>/dev/null || systemctl is-enabled --quiet ssh 2>/dev/null"
    check_item "ssh.socket не мешает (не активен)" bash -c "! systemctl is-active --quiet ssh.socket 2>/dev/null"
