...
# см. шаг 1). Вместо веб-интерфейса — карточка в NEXUS404 Hub
...
if is_done "step6_1"; then
    ...
    if [ "$(dk_beszel_mode)" = "agent" ]; then
        ...
        # решили показывать в NEXUS404 Hub только 4 метрики через
...
if is_done "step6_2"; then
    ...
    if [ -f "$BESZEL_ADMIN_EMAIL_FILE" ]; then
        ...
        echo "    (используется только для API-запросов хаба NEXUS404 Hub —"
...
if [ "$BESZEL_MODE_NOW" = "hub" ]; then
    ...
    echo "$(pad_field "Карточка в хабе:" "$FIELD_WIDTH")widget 'beszel-metrics' (см. NEXUS404 Hub)"
else
